...
const MiniCart = () => {
  ...
  useEffect(() => {
    ...
    if (isMiniCartOpen) setClosing(false)
  }, [isMiniCartOpen])

  if (!isMiniCartOpen && !closing) return null
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {items.length === 0 ? (
            <div className="text-center text-gray-500 mt-8">
              ...
              <Link href="/pets" onClick={animatedClose}>
                <button className="mt-4 px-6 py-2 bg-[#FF6B6B] text-white rounded-lg hover:bg-[#FF5555] transition-colors duration-200 cursor-pointer">
                  Tiếp tục mua sắm
                </button>
              </Link>
            </div>
                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        animatedClose()
                        setTimeout(() => router.push(`/pets/${pet.petId}`), 200)
                      }
                    }}
                    onClick={() => {
                      animatedClose()
                  ...
                  >
                    ...
                    <div className="w-16 h-16 rounded-md overflow-hidden flex-shrink-0 flex items-center justify-center">
                      <Image
                        src={item.img || '/assets/imgs/imgPet/animal-8165466_1280.jpg'}
                        alt={pet.name}
                        width={64}
                        height={64}
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm font-medium text-[#FF6B6B] truncate transition-colors duration-150 group-hover:text-[#FF5555] group-hover:underline">
                        {pet.name}
                      </h3>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <button 
                          onClick={(e) => { 
                            e.stopPropagation(); 
                            removeItem(pet.petId); 
                          }}
                          aria-label={`Xóa ${pet.name} khỏi giỏ`}
